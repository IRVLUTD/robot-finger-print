...
class PositionController(BaseGripperController):
    ...
    def forward(self, action, grippers, close_position):
        ...
        actions = ArticulationActions(joint_positions = pos,joint_velocities = self.close_mask)
        # A controller has to return an ArticulationAction
        self.last_actions = actions
        #print(pos)
        return actions
...
class TransferPositionController(BaseGripperController):
    ...
    def __init__(self, close_mask, close_position):
        name = "Controller"
        super().__init__(name=name)
        self.type = 'transfer_position_controller_v0'
        self.close_mask = close_mask
        self.touch_dofs = np.zeros_like(close_position)
        return 

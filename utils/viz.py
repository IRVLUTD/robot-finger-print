...
def plot_point_cloud(pts, color="lightblue", mode="markers"):
    return go.Scatter3d(
        ...
        z=pts[:, 2],
        mode=mode,
        marker=dict(color=color, size=3.0),
    )


occ_cmap = lambda levels, thres=0.0: [
    (
        f"rgb({int(255)},{int(255)},{int(255)})"
        if x > thres
        else f"rgb({int(0)},{int(0)},{int(0)})"
    )
...
def plot_point_cloud_occ(pts, color_levels=None):
    return go.Scatter3d(
        ...
        z=pts[:, 2],
        mode="markers",
        marker={"color": occ_cmap(color_levels), "size": 3, "opacity": 1},
    )


contact_cmap = lambda levels, thres=0.0: [
    (
        # f"rgb({int(255 * (1 - x))},{int(255 * (1 - x))},{int(255 * (1 - x))})"
        f"rgb({int(255 * (1 - x))}, {int(0)}, {int(255 * x)})"
        if x >= thres
        else f"rgb({int(0)},{int(0)},{int(0)})"
    )
    for x in levels.tolist()
...
def plot_point_cloud_cmap(pts, color_levels=None):
    return go.Scatter3d(
        ...
        z=pts[:, 2],
        mode="markers",
        marker={"color": contact_cmap(color_levels), "size": 6.5, "opacity": 1},
    )


normal_color_map = lambda levels, thres=0.0, color_scale=8.0: [
    (
        f"rgb({int(255 * (color_scale * x[0]))},{int(255 * (color_scale * x[1]))},{int(255 * (color_scale * x[2]))})"
        if x[0] >= thres
        else f"rgb({int(0)},{int(0)},{int(0)})"
    )
    for x in levels.tolist()
...
def plot_normal_map(pts, normal):
    return go.Scatter3d(
        ...
        z=pts[:, 2],
        mode="markers",
        marker={"color": normal_color_map(np.abs(normal)), "size": 3.5, "opacity": 1},
    )
    ...
    if include_contacts:
        ...
        for ind in range(contact_idx.shape[1]):
            contact_point_vertices = torch.gather(
                hand_vertices, 1, contact_idx[:, ind].unsqueeze(-1).tile((1, 1, 3))
            )
    ...
    for batch_idx in range(hand_vertices.shape[0]):
        ...
        for obj_ind, obj in enumerate(object_models):
            ...
            if include_contacts:
                to_plot.append(
                    plot_point_cloud(
                        contact_points[obj_ind][batch_idx], color=colors[obj_ind]
                    )
                )
        ...
        if save_html:
            fig.write_html(
                os.path.join(
                    f"{ directory }",
                    f"fig-{ str(uuids[ batch_idx ]) }-{ batch_idx }-{ tag }.html",
                )
            )
        if ret_plots:
            plots.append(
                torch.from_numpy(
                    np.asarray(
                        Image.open(
                            io.BytesIO(
                                fig.to_image(format="png", width=1280, height=720)
                            )
                        )
                    )
                )
...
def plot_mesh_from_name(
    dataset_object_name, color="lightblue", opacity=1.0, gdx_root="dataset/GenDexGrasp"
):
